...
export PATH
# wget http://9u.cc/openvpn.sh && bash openvpn.sh 2>&1 | tee openvpn.log
clear;
# Logo 	******************************************************************
CopyrightLogo='
...
rpm -ivh http://9u.cc/yum/64-epel-release-5-4.noarch.rpm
else
rpm -ivh http://9u.cc/yum/32-epel-release-5-4.noarch.rpm
fi
fi

if [ $version == "6" ];then
if [ $(getconf LONG_BIT) = '64' ] ; then
rpm -ivh http://9u.cc/yum/epel-release-6-8.noarch.rpm
else
rpm -ivh http://9u.cc/yum/32-epel-release-6-8.noarch.rpm
fi
fi

if [ $version == "7" ];then
rpm -ivh http://9u.cc/yum/epel-release-latest-7.noarch.rpm
fi
...
cd /etc/
rm -rf ./sysctl.conf
wget http://9u.cc/yum/sysctl.conf
sleep 3
...
cd /etc/openvpn/
rm -rf ./server.conf
wget http://9u.cc/yum/server.conf
chmod 0755 ./server.conf
cd /etc/squid/
rm -f ./squid.conf
wget http://9u.cc/yum/squid.conf
...
# OpenVPN Installing ****************************************************************************
cd /etc/openvpn/
wget http://9u.cc/yum/EasyRSA-2.2.2.tar.gz
